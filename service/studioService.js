...
var studioService = {
     ...
     */
     getStudioList:function(callback){
        chatGroup.find({valid:1,status:1,'chatStudio':{ $exists:true},groupType:constant.fromPlatform.studio}).select({chatStudio:1,name:1,level:1,groupType:1,talkStyle:1,whisperRoles:1,chatRules:1}).sort({'sequence':'asc'}).exec(function (err,rows) {
            if(err){
