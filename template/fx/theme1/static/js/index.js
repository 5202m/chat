...
var indexJS ={
    ...
    setEvent:function(){
        ...
        $(".header-right li").hover(function(){
            ...
            if($(this).is(".notice")){
                ...
                _gaq.push(['_trackEvent', 'fx_studio','header_bulletin','content_top',1,true]);
            }
        },function(){
        ...
        $(".changeroom").click(function(){
            ...
            if(pdom.hasClass("open")){
                ...
                try{
                    $.getJSON('/fxstudio/getRmCourseList',{roomIds:$(".rooms .rbox[rid]").map(function(){return $(this).attr("rid");}).get().join(",")},function(result){
        ...
        $(".rooms .rbox[rid] .enterbtn").click(function(){
            ...
            common.getJson("/fxstudio/checkGroupAuth",{groupId:thiz.attr("rid")},function(result){
                if(!result.isOK){
                    if(indexJS.checkClientGroup("vip")){
                        alert("该房间仅对新客户开放，如有疑问，请联系老师助理。");
                    }else{
                        ...
                        $.ajax({
                            ...
                            success: function (dataRt) {
                                if(dataRt.result==0){
                                    ...
                                    if(data){
                                        common.getJson("/fxstudio/modifyAvatar",{avatar:(data.fileDomain+data.filePath)}, function(result){
                                            if(!result.isOK){
    ...
    setScrollStyle:function(dom){
        dom.find(".mCSB_dragger_bar").css({background: "url(/fx/theme1/img/scroll.png) -10px 50% repeat-y",width: "6px","-webkit-border-radius": "3px", "-moz-border-radius": "3px", "border-radius": "3px"});
    },
    /**
     * 设置列表滚动条
     * @param domClass
    ...
    getArticleList:function(code,platform,hasContent,curPageNo,pageSize,orderByStr,authorId,callback){
        try{
            $.getJSON('/fxstudio/getArticleList',{authorId:common.trim(authorId),code:code,platform:platform,hasContent:hasContent,pageNo:curPageNo,pageSize:pageSize,orderByStr:orderByStr},function(data){
    ...
    getArticleInfo:function(id,callback){
        try{
            $.getJSON('/fxstudio/getArticleInfo',{id:id},function(data){
                //console.log("getArticleList->data:"+JSON.stringify(data));
                callback(data);
            });
        }catch (e){
    ...
    setAdvertisement:function(){
        this.getArticleList("advertisement",indexJS.userInfo.groupId,"0",1,5,'{"sequence":"desc","publishStartDate":"desc"}',null,function(dataList){
            if(dataList && dataList.result==0){
                var data=dataList.data;
                for(var i in data){
                    $(".ban_ul").append('<li class="swiper-slide"><a href="'+(common.isBlank(data[i].linkUrl)?"javascript:":data[i].linkUrl)+'" onclick="_gaq.push([\'_trackEvent\', \'fx_studio\', \'banner_img\', \''+data[i].detailList[0].title+'\']);"  target="_blank"><img width="100%" alt="" src="'+data[i].mediaUrl+'"></a></li>');
                    if(data.length>1){
    ...
    setTradeStrategy: function(scrollDom){
        this.getArticleList("trade_strategy_article",indexJS.userInfo.groupId,1,1,100,'{"sequence":"desc","publishStartDate":"desc"}',null,function(dataList){
            if(dataList && dataList.result==0){
