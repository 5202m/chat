...
var studioChatMbIdx={
    ...
    setAdvertisement:function(){
        this.getArticleList("advertisement","fxstudio_home","0",1,5,'{"sequence":"desc","publishStartDate":"desc"}',function(dataList){
    ...
    setBulletin : function(){
        ...
        studioChatMbIdx.getArticleList("bulletin_system","fxstudio_home",1,1,1,'{"sequence":"desc","publishStartDate":"desc"}',function(dataList){
    ...
    setEventCen : function(){
        ...
        $('.cen-ulist li').bind("click", cenTab,  function(event){
            ...
            if(type=='bulletinTab'){
                ...
                _gaq.push(['_trackEvent', 'm_fx_studio', 'newInfo_tab', 'content_middle',1,true]);
                studioChatMbIdx.setNewsFlash("#newInfoTab .boxcont");
            }
            //else if(type=='commentTab'){
            //    _gaq.push(['_trackEvent', 'm_fx_studio', 'comment_tab', 'content_middle',1,true]);
        ...
        //    _gaq.push(['_trackEvent', 'm_fx_studio', 'comment_more', 'content_middle',1,true]);
    ...
    setEventRoom:function(){
        ...
        $(".btns .enter").click(function(){
            ...
            common.getJson("/fxstudio/checkGroupAuth",{groupId:loc_groupId}, function(result){
    ...
    setEventSyllabus : function(){
        ...
        $.getJSON('/fxstudio/getSyllabus?t=' + new Date().getTime(),{groupType:studioChatMbIdx.userInfo.groupType,groupId:groupIds},function(result){
        ...
        $('.btns .timebtn').click(function(){
            ...
            _gaq.push(['_trackEvent', 'm_fx_studio', 'schedule_' + groupId, 'content_middle', 1, true]);
    ...
    getArticleList:function(code,platform,hasContent,curPageNo,pageSize,orderByStr,callback){
        try{
            $.getJSON('/fxstudio/getArticleList',{
                code:code,
                platform:platform,
    ...
    setNewsFlash : function(domId){
        ...
        $.getJSON(studioChatMbIdx.apiUrl+ '/common/getInformation', null, function(result){
            if(result){
                if(result.isOK) {
            ...
            if(data && (info=data.informations).result==0){
                ...
                if(list && list.length > 0){
                    ...
                    for(var i in list){
                        row=list[i];
                        if(!isAppend){
                            gaStr = ' onclick="_gaq.push([\'_trackEvent\', \'m_fx_studio\', \'' + gaKey + i + '\', \'content_middle\',1,true]);"'
                        }else{
                            gaStr = '';
                        }
                ...
                if(info.totalPage > info.pageNo){
                    moreDom.html("更多专业评论").attr("pno", pageNo + 1);
                }else{
                    moreDom.html("已加载全部专业评论").attr("pno", -1);
                }
            }else{
