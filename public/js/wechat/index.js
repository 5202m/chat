...
var wechat={
    ...
    setSocket:function(){
        ...
        //通知信息
        this.socket.on('outRoomNotice',function(result){
            switch (result.type)
            {
                case 'onlineNum':
                {
                    ...
                    if(data.rOnlineSizeArr){
                        var row=null,fo=null;
                        for(var i in data.rOnlineSizeArr){
                            row=data.rOnlineSizeArr[i];
                            fo=$('.expert-ul li .fj-rens[rId='+row.groupId+']');
                            fo.html(row.onlineSize);
                            var aDom=$('.expert-ul li .fxs-btn a[rId='+row.groupId+']');
                            if(aDom.attr("dw")=="true"){//开放日期允许的前提下，判断人数切换样式
                                if(parseInt(fo.next("label").text())<=parseInt(fo.text())){//最大人数小于等于当前人数，不能进入
                    ...
                    if(data.groupId){
                        ...
                        fj.html(data.rOnlineUserNum);
                        if(aDom.attr("dw")=="true"){//开放日期允许的前提下，判断人数切换样式
                            if(parseInt(fj.next("label").text())<=parseInt(fj.text())){//最大人数小于等于当前人数，不能进入
                                aDom.removeClass("enter-on").addClass("enter-off");
                            }else{
                                aDom.removeClass("enter-ff").addClass("enter-on");
                            }
                    ...
                    if(data.onlineUserNum){
                       $("#onlineUserNum").html(data.onlineUserNum);
                    }
                    break;
                }
            }
     ...
     * @returns {string}
     */
    formatOpenDate:function(openDate){
        var dateStrArr=[];
        if(common.isValid(openDate)){
            ...
            if(common.isValid(weekTime)){
                ...
                for(var i in weekTime){
                    ...
                    if(common.isValid(timeStr)){
                        dateStrArr.push(timeStr);
                    }
                }
            }
            return common.arrayUnique(dateStrArr).join(" ");
        }else{
            return "不限";
        }
    },
    ...
    setRoomList:function(activeIndex){
        try{
            ...
            $.getJSON('/wechat/getRoomList',function(data){
                ...
                if(data){
                    ...
                    for(var i in data){
                        ...
                        openDateAllow=common.dateTimeWeekCheck(row.openDate,true);
                        domArr.push('<li><aside class="fangj-ac clearfix">');
                        domArr.push('<p>'+row.name+'</p>');
                        domArr.push('<p>开放时间：'+wechat.formatOpenDate(row.openDate)+'</p>');
                ...
                $('.text-li').click(function(){
                    ...
                    wechat.getArticleInfo(id,function(result){
                        if(common.isValid(result)){
                            ...
                            wechat.setImgSwipBox($(".text-info .zw img"));
                        }
                    });
                });
                /*视频详细*/
    ...
    showBulletin : function(title,date,content){
        ...
        wechat.setImgSwipBox($(".anounce-info img"));
    }
};
// 初始化
$(function() {
